...
#ifdef CONFIG_MTK_MTD_NAND
		/* NOTE(Nelson): Because SPI_NAND do nothing for
					...
					int *busw)
{
	...
	*busw = 0;
	if (le16_to_cpu(p->features) & 1)
 	...
 	if ((mtd->oobsize > 112) && (mtd->writesize == 0x800))
		mtd->oobsize = 112;
	else if ((mtd->oobsize > 224) && (mtd->writesize == 0x1000))
		mtd->oobsize = 224;
						  ...
						  const struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	if (chip->chipsize ==0)//added by bayi
