...
function __mtkwifi_reload(devname)
    ...
    for dev,profile in pairs(profiles) do
        if not devname or devname == dev then
            ...
            -- so we call "wifi restart" here.
            assert(type(diff) == "table")
            if diff.BssidNum then
                os.execute("wifi restart "..devname)
            else
                os.execute("wifi reload "..devname)
            end
...
function dev_cfg_reset(devname, load_overview_webpage)
    -- http.write_json(http.formvalue())
    local profiles = mtkwifi.search_dev_and_profile()
    assert(profiles[devname])
...
function vif_cfg(dev, vif) 
    ...
    if http.formvalue("__apply") then
        cfgs.WscConfStatus = mtkwifi.token_set(cfgs.WscConfStatus,vif_idx, "2")
    end
    ...
    if(http.formvalue("__hidessid") == "1") then
        cfgs.WscConfMode = mtkwifi.token_set(cfgs.WscConfMode, vif_idx, "0")
    end
    if(((http.formvalue("__authmode")=="OPEN") and
        (http.formvalue("__wep_encrypttype") == "WEP")) or
       (http.formvalue("__hidessid") == "1")) then
        os.execute("iwpriv "..vifname.." set WscConfMode=0")
        debug_write("iwpriv "..vifname.." set WscConfMode=0")
    end
...
function set_wifi_gen_pin(ifname,devname)
    ...
    -- Find out the difference in profile settings before saving the new profile settings which will be used
    -- to decide whether the display of "need reload to apply changes" message is required or not.
    local diff = mtkwifi.diff_profile(profile)
    mtkwifi.save_profile(cfgs, profile)
...
function get_apcli_conn_info(ifname)
    ...
    if not ifname then
        ...
        local _fIdx,_lIdx,ssid = string.find(iwapcli, "ESSID:\"(.*)\"")
        if not ssid or ssid == "" then
            rsp["conn_state"]= "Disconnected"
        else
            rsp["conn_state"] = "Connected"
            rsp["ssid"] = ssid
            rsp["bssid"] = mtkwifi.read_pipe("cat /sys/class/net/"..ifname.."/address 2>/dev/null") or "?"
        end
    end
    http.write_json(rsp)
end
...
function apcli_connect(dev, vif)
    ...
    -- Find out the difference in profile settings before saving the new profile settings which will be used
    -- to decide whether the display of "need reload to apply changes" message is required or not.
    local diff = mtkwifi.diff_profile(profiles[devname])
    mtkwifi.save_profile(cfgs, profiles[devname])
    if next(diff) == nil then
        os.execute("cp -f "..profiles[devname].." "..mtkwifi.__profile_applied_settings_path(profiles[devname]))
    end
    ...
    if cfgs.ApCliEncrypType == "WEP" then
        ...
        if (cfgs.ApCliDefaultKeyID == "1") then
            ...
            debug_write("#iwpriv "..vifname.." set ApCliKey1="..cfgs.ApCliKey1Str)
        elseif (cfgs.ApCliDefaultKeyID == "2") then
            os.execute("#iwpriv "..vifname.." set ApCliKey2="..cfgs.ApCliKey2Str)
            debug_write("#iwpriv "..vifname.." set ApCliKey2="..cfgs.ApCliKey2Str)
        elseif (cfgs.ApCliDefaultKeyID == "3") then
            os.execute("#iwpriv "..vifname.." set ApCliKey3="..cfgs.ApCliKey3Str)
            debug_write("#iwpriv "..vifname.." set ApCliKey3="..cfgs.ApCliKey3Str)
        elseif (cfgs.ApCliDefaultKeyID == "4") then
            os.execute("#iwpriv "..vifname.." set ApCliKey4="..cfgs.ApCliKey4Str)
            debug_write("#iwpriv "..vifname.." set ApCliKey4="..cfgs.ApCliKey4Str)
        end
    elseif cfgs.ApCliAuthMode == "WPAPSK"
        or cfgs.ApCliAuthMode == "WPA2PSK"
        or cfgs.ApCliAuthMode == "WPA1PSKWPA2PSK" then
