...
int getCurrentWscProfile(lua_State *L)
{
	...
	data = malloc(sizeof( WSC_CONFIGURED_VALUE));
	...
	if (ioctl(socket_id, RTPRIV_IOCTL_WSC_PROFILE, &wrq) < 0) {
		printf("ioctl -> RTPRIV_IOCTL_WSC_PROFILE Fail !");
		close(socket_id);
		return -1;
	}
	...
	//1. configured state
	//printf("WSC configured %d\n", data->WscConfigured);
	//2. WPSSSID
	if(strchr(data->WscSsid, '\n')){
	...
	//printf("value for status %d", status);
...
int StaInfo(lua_State *L)
{
	...
	for (i = 0; i < table.Num; i++) {
		...
		snprintf(tmpBuff, sizeof(tmpBuff), "%d", pe->TxRate.field.BW? 40: 20);
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Bw");
		...
		snprintf(tmpBuff, sizeof(tmpBuff), "%d", ((lastRxRate>>7) & 0x1)? 40: 20);
		lua_pushstring(L, tmpBuff);
		...
		snprintf(tmpBuff, sizeof(tmpBuff), "%s", phyMode[(lastRxRate>>14) & 0x3]);
		lua_pushstring(L, tmpBuff);
