				     ...
				     struct foe_entry *foe)
{
...
#if defined(CONFIG_NET_DSA) || defined(CONFIG_NET_MEDIATEK_HW_QOS)
	int lan = IS_LAN(dev);
#endif
	struct ethhdr *eth;
	struct iphdr *iph;
	...
	//printk("skb_to_hnat_info tcp=%d , ipv4=%d etype=%x \n",tcp,ipv4,entry.ipv4_hnapt.etype);
	...
	if ( IS_DSA_LAN(dev) ){
		...
		if( (host->wan_dsa_port != NONE_DSA_PORT) && (host->wan_dsa_port == 0) ) {
			/* wllll : wan at port0 , lan0 at port1 */
			entry.ipv4_hnapt.etype = htons(BIT((dev->name[3] - '0')+1));
                }
		else {
                	/* llllw : lan0 at port0 , wan/eth1(gphy) at port4 */
			entry.ipv4_hnapt.etype = htons(BIT(dev->name[3] - '0'));
		}
		...
		entry.ipv4_hnapt.etype = htons(BIT(host->wan_dsa_port));
	}
